...
/*The following code initializes the accelerometer -->    by writing specific values to the control registers used below*
*and also reads the acceleration values along the X, Y, Z axes*/
...
{	
	...
	/*Settimg up the GSCALE value to measure acceleration in the -2g to +2g range*/	
	fsr = GSCALE;
	if(fsr > 8) fsr = 8;
	fsr = fsr >> 2;
...
void readAcc_XYZ(void)
{
	...
	/*X, Y and Z values are 16 bit values*/
